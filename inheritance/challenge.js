class Character {
  ...
  constructor(name) {
    this.name = name;
    this.#health = 100;
    Character.incrementCharacterCount();
  }
  static incrementCharacterCount() {
  ...
  set health(hp) {
    // if (hp < 0) throw new Error("Health cant be below 0");
  ...
  getStatus() {
    return `name: ${this.name}, health: ${this.health}, status: ${this.aliveStatus ? "alive" : "dead"}`;
  }
...
class Hero extends Character {
  ...
  getItems() {
    return `${this.name} has the following  items: ${this.inventories.join(", ")}`;
  }
  /* The Hero class
