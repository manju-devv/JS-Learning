...
// bookKeys.forEach(item => console.log(item));

// bookKeys.forEach(item => console.log(books[item].title))
...
// const bookValues = Object.values(books);
// bookValues.forEach(item => console.log(item.price))
...
bookEntries.filter(([bookId,bookData]) => bookData.price < 16).forEach(([id,data]) => {
    console.log(`ID: ${id}, Book: ${data.title}, Price: £${data.price}`)
})
